...
def make_stepwiseMLPVAE(params, max_params):
    ...
    model_text = f"{model_type} control={hidden_size}, depth={depth}, ratio={ratio:.2f}, VAE latent={latent_size}, depth={vae_depth}, ratio={vae_ratio:.2f}"
    print(model_text)
...
def make_RNN_VAE(model_type, model_params, max_params):
    ...
    model_text = f"{model_type} hidden={hidden_size}, encode_depth={encode_depth}, decode_depth={decode_depth}, VAE latent={latent_size}, depth={vae_depth}, ratio={vae_ratio:.2f}"
    print(model_text)
...
def make_Conv2D_VAE(model_type, model_params, max_params):
    ...
    model_text = f"{model_type} conv layers={layer_count}, kernels={kernel_count}, size={kernel_size}, VAE latent={latent_size}, depth={vae_depth}, ratio={vae_ratio:.2f}"
    print(model_text)
...
def make_model(model_type, model_params, max_params, verbose):
    ...
    match model_type:
        ...
        case "MLP_VAE":
            ...
            model_text = f"{model_type} hidden={hidden_size}, depth={mlp_depth}, ratio={mlp_ratio:.1f}, VAE latent={latent_size}, depth={vae_depth}, ratio={vae_ratio:.2f}"
            print(model_text)
        ...
        case "AudioConv_VAE":
            model, model_text, approx_size, vae_size = make_AudioConv_VAE(model_type, model_params, max_params)
            if is_too_large(approx_size, max_params):
                return invalid_model(approx_size)
